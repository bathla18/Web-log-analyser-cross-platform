...
int Config_debug = 0;
int Config_max_referers = 200;
int Config_max_pages = 200;
int Config_max_images = 200;
int Config_max_error404 = 200;
int Config_max_agents = 200;
int Config_max_googled = 200;
int Config_max_google_keyphrases = 200;
int Config_max_trails = 200;
int Config_max_tld = 200;
...
int vi_is_googlebot_agent(char *agent) {
	if (strstr(agent, "Googlebot") || strstr(agent, "googlebot")) return 1;
        return 0;
}
...
int vi_is_bot_agent(char *agent) {
    if (vi_is_googlebot_agent(agent) || vi_is_adsensebot_agent(agent) || vi_is_yahoobot_agent(agent) ||
...
int vi_is_internal_link(char *url)
{
	...
	for (i = 0; i < Config_prefix_num; i++) {
		if (Config_prefix[i].len <= l &&
		    !strncasecmp(url, Config_prefix[i].str,Config_prefix[i].len))
		{
...
struct vih *vi_new(void)
{
	...
	vih->invalid = 0;
 	vih->blacklisted = 0;
	vi_reset_combined_maps(vih);   //reset weekday hours
	vih->error = NULL;
...
void vi_process_date_and_hour(struct vih *vih, int weekday, int hour)
{
	...
	vih->weekday[weekday]++;
	vih->hour[hour]++;
	/* store the combined info. Always compute this information
 ...
 * Return non-zero on out of memory. */
int vi_process_referer(struct vih *vih, char *ref)
{
	int res;
...
int vi_process_google_keyphrases(struct vih *vih, char *ref)
{
	char *s, *p, *e;
	int res, page;
	char urldecoded[VI_LINE_MAX];
...
int vi_process_line(struct vih *vih, char *l)
{
	...
	if (vi_parse_line(&ll, l) == 0) {
		...
		if (Config_process_google_keyphrases &&
		    vi_process_google_keyphrases(vih, ll.ref))
			goto oom;
		...
		if (vi_process_referer(vih, ll.ref)) goto oom;
...
{
	fprintf(fp, "Statistics generated with WSLA version %s\n",
	       VI_VERSION_STR);
}
...
{
	fprintf(fp, "<tr><td colspan=\"3\" align=\"center\" class=\"credits\">Statistics generated with <a href=\""">WSLA </a> version %s\n</td></tr>", VI_VERSION_STR);
}
 ...
 * (usually url and date). Used to print the 'googled'  */
...
/* Generate the report writing it to the output file 'of'. If op is NULL, output the report to standard output.
 * On success zero is returned. Otherwise the function returns non-zero and set an error in the vih handler. */
...
enum { OPT_MAXREFERERS, OPT_MAXPAGES, OPT_MAXIMAGES, OPT_USERAGENTS, OPT_ALL, OPT_MAXLINES, OPT_GOOGLE, OPT_MAXGOOGLED, OPT_MAXUSERAGENTS, OPT_VERSION, OPT_HELP, OPT_PREFIX, OPT_TRAILS, OPT_GOOGLEKEYPHRASES,  OPT_MAXGOOGLEKEYPHRASES, OPT_MAXTRAILS, OPT_GRAPHVIZ, OPT_WEEKDAYHOUR_MAP, OPT_MONTHDAY_MAP,  OPT_TAIL, OPT_TLD, OPT_MAXTLD, OPT_STREAM, OPT_UPDATEEVERY, OPT_RESETEVERY, OPT_OS, OPT_BROWSERS, OPT_ERROR404, OPT_MAXERROR404, OPT_TIMEDELTA, OPT_PAGEVIEWS, OPT_GRAPHVIZ_ignorenode_GOOGLE, OPT_GRAPHVIZ_ignorenode_EXTERNAL, OPT_GRAPHVIZ_ignorenode_NOREFERER, OPT_GOOGLEHUMANLANGUAGE, OPT_FILTERSPAM,  OPT_GREP, OPT_EXCLUDE, OPT_IGNORE404, OPT_DEBUG};
...
static struct ago_optlist wsla_optlist[] = {
...
/*	{ '\0', "stream",		OPT_STREAM,		AGO_NOARG},
   ...
   /* { '\0', "exclude",              OPT_EXCLUDE,            AGO_NEEDARG},*/
	...
	{ '\0', "graphviz-ignorenode-google", OPT_GRAPHVIZ_ignorenode_GOOGLE,AGO_NOARG},
	{ '\0', "graphviz-ignorenode-external", OPT_GRAPHVIZ_ignorenode_EXTERNAL,AGO_NOARG},
	{ '\0', "graphviz-ignorenode-noreferer", OPT_GRAPHVIZ_ignorenode_NOREFERER, AGO_NOARG},
...
void wsla_show_help(void)
{
	int i;

	printf("Usage: WSLA [options] <filename> [<filename> ...]\n");
	printf("Available options:\n");
	for (i = 0; wsla_optlist[i].ao_long != NULL; i++) {
		if (wsla_optlist[i].ao_short != '\0') {
			printf("  -%c ", wsla_optlist[i].ao_short);
		} else {
			printf("     ");
		}
		printf("--%-30s %s\n",wsla_optlist[i].ao_long,(wsla_optlist[i].ao_flags & AGO_NEEDARG) ?"<argument>" : "");
	}
}
...
int main(int argc, char **argv)
{
	...
	while((o = antigetopt(argc, argv, wsla_optlist)) != AGO_EOF) {
		switch(o) {
		case AGO_UNKNOWN:
		case AGO_REQARG:
		case AGO_AMBIG:
			ago_gnu_error("WSLA", o);
			wsla_show_help();
			exit(1);
			break;
		case OPT_HELP:
		...
		case OPT_VERSION:
			printf("WSLA %s\n", VI_VERSION_STR);
			exit(0);
		case OPT_MAXREFERERS:    //Set the max number of entries in the referer report.
			Config_max_referers = atoi(ago_optarg);
			break;
		case OPT_MAXPAGES:       //Set the max number of entries in the accessed pages report.
			Config_max_pages = atoi(ago_optarg);
			break;
		case OPT_MAXIMAGES:      //Set the max number of entries in the accessed images report.
			Config_max_images = atoi(ago_optarg);
			break;
		case OPT_MAXERROR404:    //Set the max number of entries in the missing documents report.
			Config_max_error404 = atoi(ago_optarg);
			break;
		case OPT_MAXUSERAGENTS:  //Set the max number of entries in the user agents report.
			Config_max_agents = atoi(ago_optarg);
			break;
		case OPT_MAXTRAILS:
			Config_max_trails = atoi(ago_optarg);
			break;
		case OPT_MAXGOOGLED:      //Set the max number of entries in the crawled pages report (google bot).
			Config_max_googled = atoi(ago_optarg);
			break;
		case OPT_MAXGOOGLEKEYPHRASES:    //Set the max number of entries in the Google keyphrases report.
			Config_max_google_keyphrases = atoi(ago_optarg);
			break;
		case OPT_MAXTLD:        //Set the max number of entries in the domains report.
			Config_max_tld = atoi(ago_optarg);
			break;
		case OPT_USERAGENTS:      //Show information about common user agents.
			Config_process_agents = 1;
			break;
		case OPT_GOOGLE:          //Activate report about pages accessed by the Google
			Config_process_google = 1;
			break;
		case OPT_GOOGLEKEYPHRASES:   //Activate report that shows common search keyphrases used to found web site from Google
			Config_process_google_keyphrases = 1;
			break;
		case OPT_GOOGLEHUMANLANGUAGE: //Activate a report that shows common human languages used
                                        //to search from Google. This feature uses the 'hl' variable of the Google referer URL.
            Config_process_google_keyphrases = 1;
			Config_process_google_human_language = 1;
			break;
		case OPT_TLD:             //Activate the generation of information about Top Level Domains popularity. This information may be useful to guess the amount
                                // of visits from different countries. Note that WSLA will not resolve numerical IP addresses
                            //if they are not already resolved in the log file. All the unresolved IP addresses will be shown in this
	...
	/* If the user specified the 'tail' mode, WSLA
