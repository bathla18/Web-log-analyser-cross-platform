...
namespace WSLA
{
	...
	public class DailyTrafficListView : QueryListView
	{
		...
		{
			String Result =
                "SELECT DISTINCT DateSerial(Year([DT]),Month([DT]),Day([DT])) AS [Date], Sum([Bytes]) AS [Downloads (bytes)], [IPAddress] AS [IP Address or Hostname]\n" +
				"FROM HTTPInfo\n" +
				"WHERE [HTTPInfo].[DT] >= " + GetSQLDate(Date1) + " AND [HTTPInfo].[DT] < " + GetSQLDate(Date2.AddDays(1)) + "\n" +
                "GROUP BY DateSerial(Year([DT]),Month([DT]),Day([DT])),[IPAddress]\n" +
